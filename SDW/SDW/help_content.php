...
<li style="margin-left:30px"><a href="#AutomatedInvitations" style="font-family:Verdana">Surveys: Automated Survey Invitations &amp; Survey Queue</a></li>
...
Depending on your the project settings enabled, you will have the following steps/modules to complete on the Project Set-up page:
</p>
<table class="wiki">
...
<p>
The survey-related options, like Survey Settings and Notifications, can be access on the Project Setup &gt; Online Designer page.
</p>
<p>
...
<b><font color="#600000">Q: Do parent/child projects allow 1:many (one to many) relationships, were an unknown number of possible responses can be handled for the same question? </font></b>
<p>
...
<h1 id="AutomatedInvitations"><font color="#600000">Automated Survey Invitations &amp; Survey Queue</font><a class="anchor" href="#AutomatedInvitations" title="Link to this section"> ¶</a></h1>
<p>
For any survey in your REDCap project, you may define your conditions for Automated Survey Invitations that will be sent out for a specified survey. This is done on the Online Designer page. Automated survey invitations may be understood as a way to have invitations sent to your survey participants, but rather than sending or scheduling them manually via the Participant List, the invitations can be scheduled to be sent automatically (i.e. without a person sending the invitation) based upon specific conditions, such as if the participant completes another survey in your project or if certain data values for a record are fulfilled. Also, conditional logic can be set in the same way for the Survey Queue, which is a list of surveys displayed all on one page to a participant, in which the queue comprises all surveys that are to be completed (i.e., a 'to-do' list) as well as the surveys that the participant has already completed. <strong>The syntax for the conditional logic is exactly the same for both Automated Survey Invitations and the Survey Queue</strong>, so the documentation below is valid for both of these features.
</p>
<b><font color="#600000">Q: What mathematical operations can be used in the logic for Automated Survey Invitations/Survey Queue? </font></b>
<table class="wiki">
<tbody><tr><td>+        </td><td>Add
</td></tr><tr><td>-        </td><td>Subtract
</td></tr><tr><td>*        </td><td>Multiple
...
<b><font color="#600000">Q. What functions can be used in the logic for Automated Survey Invitations/Survey Queue?</font></b>
<p>
The Automated Survey Invitations/Survey Queue functionality can perform many advanced functions. A complete list is listed below. (NOTE: These functions are very similar - and in some cases identical - to functions that can be used for calculated fields.)
</p>
<table class="wiki">
<tbody><tr><td> <strong>Function</strong> </td><td> <strong>Type of calculation</strong> </td><td> <strong>Notes / examples</strong>
</td></tr><tr><td> round(number,decimal places) </td><td> <strong>Round</strong> </td><td> If the "decimal places" parameter is not provided, it defaults to 0. E.g. To round 14.384 to one decimal place:  round(14.384,1) will yield 14.4 
...
<b><font color="#600000">Q: Can I use conditional IF statements in the logic for Automated Survey Invitations/Survey Queue? </font></b>
<p>
...
<b><font color="#600000">Q: How can I calculate the difference between two date or time fields in my conditional logic for Automated Survey Invitations/Survey Queue (this includes time, datetime, and datetime_seconds fields)? </font></b>
<p>
You can calculate the difference between two dates or times by using the datediff function shown below. <strong>IMPORTANT NOTE:</strong> The Automated Survey Invitation/Survey Queue datediff function differs from the datediff function used in calculated fields in that the ASI/SQ datediff does NOT have a "dateformat" parameter. This is due to the fact that the date format of the dates used in the ASI datediff function does not have to be specified, whereas it does for calc fields.
</p>
<p>
...
<b><font color="#600000">Q: Can I base my Automated Survey Invitation/Survey Queue's datediff function off of today's date? </font></b>
<p>
...
<b><font color="#600000">Q: Can I use the same format of the datediff function for Automated Survey Invitation/Survey Queue that is used for calculated fields, which requires the dateFormat ("ymd", "mdy", or "dmy") as the fourth parameter? </font></b>
<p>
Yes, you can use the calculated field version of the datediff function. If the fourth parameter of the datediff function for Automated Survey Invitation/Survey Queue is "ymd", "mdy", or "dmy", it will ignore it (because it is not needed) and will then assume the fifth parameter (if provided) to instead be the returnSignedValue.
</p>
...
<p>
NOTE: The check of the conditions is _ONLY_ done when data for a record are saved. Conditions, therefore, that depend on a certain time interval having elapsed (such as a patient reaching a certain age, or a certain number of weeeks since the first visit elapsing) can easily be missed, causing no survey invitation to be scheduled.
</p>
<b><font color="#600000">Q: How can I make sure that automated survey invitations that depend on a certain time interval having elapsed will be sent? </font></b>
<p>
Possibilities include:
</p>
<p>
* Manually save all records every day.
</p>
<p>
* Create a program that uses the REDCap API to automatically save all records every day.
</p>
...
<p>
&lt;b&gt;Note:&lt;/b&gt; Although setting up a Data Quality custom rule may at times be very similar to constructing an equation for a calculated field, calc fields will ALWAYS have to result in a number, whereas the &lt;b&gt;Data Quality custom rule must ALWAYS result with a TRUE or FALSE condition and NEVER a value.&lt;/b&gt;
</p>
...
<b><font color="#600000">Q: My datediff function returns a number and not a true or false statement. How do I incorporate it in a Data Quality rule without getting an error message.</font></b>
<p>
Try putting your datediff function in an if-statement.
For example:
if((datediff([date1], [date2], "units", returnSignedValue))&lt; 300, true, false)
</p>
...
<b><font color="#600000">Q: Can I use the same format of the Data Quality datediff function that is used for calculated fields, which requires the dateFormat ("ymd", "mdy", or "dmy") as the fourth parameter? </font></b>
<p>
Yes, you can use the calculated field version of the datediff function. If the fourth parameter of the Data Quality datediff function is "ymd", "mdy", or "dmy", it will ignore it (because it is not needed) and will then assume the fifth parameter (if provided) to instead be the returnSignedValue.
</p>
<p>
<br>
</p>
